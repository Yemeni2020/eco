        ...
        <section class="bg-white py-6">
            <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-10">
                <div class="flex flex-wrap items-end justify-between gap-4">
                    <div>
                        <h2 class="text-2xl sm:text-3xl font-extrabold tracking-tight">Trending products</h2>
                        <p class="mt-1 text-sm text-zinc-600">Fresh picks customers are loving this week.</p>
                    </div>
                    <div class="flex items-center gap-4">
                        <div class="hidden sm:flex items-center gap-2">
                            <button id="trendingProductsPrev"
                                class="grid h-10 w-10 place-items-center rounded-full border border-zinc-200 bg-white text-zinc-900 shadow-sm hover:bg-zinc-50 active:scale-[0.98]"
                                aria-label="Previous products" type="button">&lt;</button>
                            <button id="trendingProductsNext"
                                class="grid h-10 w-10 place-items-center rounded-full border border-zinc-200 bg-white text-zinc-900 shadow-sm hover:bg-zinc-50 active:scale-[0.98]"
                                aria-label="Next products" type="button">&gt;</button>
                        </div>
                        <a href="#"
                ...
                @if ($featuredProducts->isNotEmpty())
                    <section class="relative mt-6">
                        ...
                        <div id="trendingProductsViewport"
                            class="no-scrollbar flex snap-x snap-mandatory gap-4 overflow-x-auto scroll-smooth pb-2 [scrollbar-width:none] [-ms-overflow-style:none] cursor-grab active:cursor-grabbing"
                            aria-roledescription="carousel" aria-label="Trending products carousel" tabindex="0">
                                        ...
                                        <div class="p-4">
                                            ...
                                            @if (!empty($product['rating']) && $product['rating'] > 0)
                                                <div class="mt-2 flex items-center gap-2">
                                                    ...
                                                    <span class="text-xs text-zinc-500">({{ $product['reviews'] ?? 0 }})</span>
                                                </div>
                                            @endif
                                            <div class="mt-3 flex items-center justify-between">
                                                <div class="flex items-baseline gap-2">
                                                    <span class="text-base font-extrabold text-zinc-900">
                                                        <x-currency
                                                            :amount="number_format($product['price'] ?? 0, 2)" />
                                                    </span>
                                                    @if (!empty($product['compareAt']))
                                                        <span class="text-sm text-zinc-400 line-through">
                                                            <x-currency
                                                                :amount="number_format($product['compareAt'], 2)" />
                                                        </span>
                                                    @endif
                                                </div>
                                                <button 
                        ...
                        <div class="mt-4 flex sm:hidden items-center justify-center gap-2">
                            <button id="trendingProductsPrevM"
                                class="grid h-10 w-10 place-items-center rounded-full border border-zinc-200 bg-white text-zinc-900 shadow-sm hover:bg-zinc-50 active:scale-[0.98]"
                                aria-label="Previous products" type="button">&lt;</button>
                            <button id="trendingProductsNextM"
                                class="grid h-10 w-10 place-items-center rounded-full border border-zinc-200 bg-white text-zinc-900 shadow-sm hover:bg-zinc-50 active:scale-[0.98]"
                                aria-label="Next products" type="button">&gt;</button>
                        </div>

                        <div id="trendingProductsDots" class="mt-4 flex items-center justify-center gap-2"></div>
                        <div id="trendingProductsLive" class="sr-only" aria-live="polite" aria-atomic="true"></div>
                    </section>
                @else
                    <div class="mt-6 text-sm text-zinc-500">No products available.</div>
                @endif
            </div>
        </section>
...
@push('scripts')
    ...
    <script>
        (() => {
            const viewport = document.getElementById("trendingProductsViewport");
            if (!viewport) return;

            const prev = document.getElementById("trendingProductsPrev");
            const next = document.getElementById("trendingProductsNext");
            const prevM = document.getElementById("trendingProductsPrevM");
            const nextM = document.getElementById("trendingProductsNextM");
            ...
            [prev, prevM].forEach((button) => {
                button?.addEventListener("click", () => scrollByCards(-1));
            });
            [next, nextM].forEach((button) => {
                button?.addEventListener("click", () => scrollByCards(1));
            });
        })();
    </script>
