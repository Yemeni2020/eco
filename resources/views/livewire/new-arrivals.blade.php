<section class="bg-white py-6" data-infinite-scroll>
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div class="flex items-end justify-between gap-4">
            <div>
                <h2 class="text-2xl sm:text-3xl font-extrabold tracking-tight">New Arrivals</h2>
                <p class="mt-1 text-sm text-zinc-600">Check out our latest premium accessories</p>
            </div>
        ...
        <section class="relative mt-6">
            ...
            <div id="newArrivalsViewport"
                ...
                aria-roledescription="carousel" aria-label="New arrivals carousel" tabindex="0">
                @foreach ($this->visibleProducts as $product)
                    <article class="slide snap-start shrink-0 w-[220px] sm:w-[240px] md:w-[260px]"
                        ...
                        <div
                            ...
                            <a href="{{ route('shop.show', $product['id']) }}" class="absolute inset-0 z-10"
            ...
            <div class="mt-4 flex sm:hidden items-center justify-center gap-2">
                <button id="newArrivalsPrevM"
                    class="grid h-10 w-10 place-items-center rounded-full border border-zinc-200 bg-white text-zinc-900 shadow-sm hover:bg-zinc-50 active:scale-[0.98]"
                    aria-label="Previous products" type="button">&lt;</button>
                <button id="newArrivalsNextM"
                    class="grid h-10 w-10 place-items-center rounded-full border border-zinc-200 bg-white text-zinc-900 shadow-sm hover:bg-zinc-50 active:scale-[0.98]"
                    aria-label="Next products" type="button">&gt;</button>
            </div>

            <div id="newArrivalsDots" class="mt-4 flex items-center justify-center gap-2"></div>

            <div id="newArrivalsLive" class="sr-only" aria-live="polite" aria-atomic="true"></div>
        </section>
        @if ($this->hasMore)
