    ...
    <div class="flex w-full flex-1 flex-col gap-6">
        ...
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div class="flex flex-wrap gap-3">
                <flux:button variant="outline" type="button">Discard</flux:button>
                <flux:button variant="primary" icon="check" icon:variant="outline" type="submit" form="settings-form">
                    Save changes
                </flux:button>
            </div>
        </div>
        ...
        <div class="flex flex-wrap gap-3 rounded-full border border-slate-200 bg-white px-4 py-2 shadow-inner" role="tablist">
            <button type="button" data-tab-target="store"
            ...
            <button type="button" data-tab-target="shipping"
                class="tab-toggle rounded-full px-4 py-2 text-sm font-semibold text-slate-800 transition hover:bg-slate-100 focus:outline-none">
                Shipping
            </button>

            <button type="button" data-tab-target="operations"
                class="tab-toggle rounded-full px-4 py-2 text-sm font-semibold text-slate-800 transition hover:bg-slate-100 focus:outline-none">
                Operations
            </button>
        ...
        <form id="settings-form" class="grid gap-6 xl:grid-cols-[2fr_1fr]">
            ...
            <div class="flex flex-col gap-6" data-col="left">
                ...
                {{-- Shipping --}}
                <div class="tab-panel hidden rounded-xl border border-zinc-200 bg-white p-6 shadow-xs dark:border-zinc-700 dark:bg-zinc-900" data-tab="shipping">
                    <div class="flex flex-col gap-4">
                ...
                {{-- Notifications --}}
                <div class="tab-panel hidden rounded-xl border border-zinc-200 bg-white p-6 shadow-xs dark:border-zinc-700 dark:bg-zinc-900" data-tab="notifications">
                    <div class="flex flex-col gap-4">
                ...
                {{-- SEO (moved into tabs) --}}
                <div class="tab-panel hidden rounded-xl border border-zinc-200 bg-white p-6 shadow-xs dark:border-zinc-700 dark:bg-zinc-900" data-tab="seo">
                    <div class="flex items-center justify-between">
                        ...
                        <flux:button variant="primary" type="submit" formaction="{{ route('admin.settings.seo') }}" formmethod="POST">
                            Save SEO
                        </flux:button>
                    </div>
                    ...
                    <div class="grid gap-4 lg:grid-cols-2 mt-6">
                        <flux:input name="title" label="SEO title" value="{{ old('title', $seoSetting->title) }}" />
                        <flux:input name="route_name" label="Route name (optional)" value="{{ old('route_name', $seoSetting->route_name) }}" placeholder="pages.home" />
                    </div>

                    <div class="mt-4">
                        <flux:textarea name="description" label="Description" rows="3">{{ old('description', $seoSetting->description) }}</flux:textarea>
                    </div>

                    <div class="grid gap-4 mt-4 lg:grid-cols-2">
                        <flux:input name="image" label="Default image URL" type="url" value="{{ old('image', $seoSetting->image) }}" />
                        <flux:input name="locale" label="Locale" value="{{ old('locale', $seoSetting->locale) }}" placeholder="en_US" />
                    </div>
            ...
            <div class="flex flex-col gap-6 hidden xl:block" data-col="right">
                <div class="tab-panel hidden rounded-xl border border-zinc-200 bg-white p-6 shadow-xs dark:border-zinc-700 dark:bg-zinc-900" data-tab="operations">
                    <div class="flex flex-col gap-4">
                        ...
                        <div class="grid gap-4">
                            ...
                            <div class="rounded-2xl border border-zinc-200 bg-white p-4">
                                <h3 class="mb-3 text-sm font-semibold text-slate-700">Social media login</h3>
...
<script>
  function initSettingsTabs() {
    ...
    const leftCol = document.querySelector('[data-col="left"]');
    const rightCol = document.querySelector('[data-col="right"]');
    ...
    const setActive = (target) => {
      ...
      // columns visibility (operations right)
      if (leftCol && rightCol) {
        const isOperations = target === 'operations';

        // on XL show right column only for operations
        leftCol.classList.toggle('hidden', isOperations);
        rightCol.classList.toggle('hidden', !isOperations);

        // keep right column visible on xl when operations active
        rightCol.classList.toggle('xl:block', isOperations);
      }
    };
