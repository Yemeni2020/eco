...
class ApplePayGateway implements PaymentGateway
{
    ...
    {
        $reference = 'APPLEPAY-' . Str::upper(Str::random(10));
        $redirectUrl = $this->buildRedirectUrl($order, $reference);

        return new PaymentInitResult('applepay', $reference, $redirectUrl, [
            'merchant_id' => $this->config['merchant_id'] ?? null,
            'apple_merchant_id' => $this->config['apple_merchant_id'] ?? null,
    ...
    {
        $base = str_replace('{provider}', 'applepay', config('payments.return_url'));
